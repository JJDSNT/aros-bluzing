 ...
 *   - No per-request timeout: unlike bt_sdp_client (whose underlying
 *     L2CAP channel has an RTX timer during connection setup), the fixed
 *     ATT channel has no connection handshake and thus no natural place
 *     to hang a timeout on individual requests. A stalled server is only
 *     detected if/when the ACL link itself goes away (channel CLOSED).
 *   - GATT Server, and anything beyond Exchange MTU/discovery/Read/
 *     Write/notifications (e.g. Write Without Response, Reliable
 *     Writes, Long Read/Write) aren't implemented.
 */
