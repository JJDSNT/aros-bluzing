...
- [`deteccao-adaptador-firmware-propostas.md`](deteccao-adaptador-firmware-propostas.md) — propostas de desenho para como cada port sabe que um adaptador está disponível (`psdAddEventHandler` do Poseidon para x64 hotplugável via USB, device tree/HCI para a revisão de chip no Raspberry Pi) e como tratar upload de firmware vendor-specific (Broadcom/Intel/Realtek/Qualcomm) como comandos HCI vendor-specific, com tabela em runtime em vez de builds separados por chipset.
